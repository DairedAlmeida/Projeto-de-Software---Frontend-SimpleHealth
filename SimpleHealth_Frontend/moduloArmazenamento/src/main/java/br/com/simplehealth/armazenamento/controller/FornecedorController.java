...
public class FornecedorController extends AbstractCrudController<Fornecedor, br.com.simplehealth.armazenamento.view.Fornecedor, Long> {
    ...
    private void buscarPorNome() {
        if (nomeField != null && !nomeField.getText().trim().isEmpty()) {
            try {
                List<Fornecedor> fornecedores = fornecedorService.buscarPorNome(nomeField.getText().trim());
                
                javafx.collections.ObservableList<br.com.simplehealth.armazenamento.view.Fornecedor> viewModels = 
                    javafx.collections.FXCollections.observableArrayList();
                
                for (Fornecedor fornecedor : fornecedores) {
                    viewModels.add(modelToView(fornecedor));
                }
                ...
                logger.error("Erro ao buscar fornecedores por nome: ", e);
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Erro");
                alert.setContentText("Erro ao buscar fornecedores: " + e.getMessage());
                alert.showAndWait();
            }
        }
    }
    ...
    /**
     * Limpa o filtro e recarrega todos os fornecedores.
     */
    @FXML
    private void limparFiltro() {
        ...
        try {
            List<Fornecedor> fornecedores = buscarTodosViaApi();
            javafx.collections.ObservableList<br.com.simplehealth.armazenamento.view.Fornecedor> viewModels = 
                javafx.collections.FXCollections.observableArrayList();
            ...
            logger.error("Erro ao recarregar fornecedores: ", e);
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Erro");
            alert.setContentText("Erro ao recarregar fornecedores: " + e.getMessage());
            alert.showAndWait();
        }
    }
}